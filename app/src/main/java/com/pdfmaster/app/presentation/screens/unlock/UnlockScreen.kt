...
fun UnlockScreen(onBack: () -> Unit, viewModel: UnlockViewModel = hiltViewModel()) {
    ...
    ) { uri -> uri?.let { viewModel.unlock(it, password) } }
    ...
    LaunchedEffect(state) {
        when (val current = state) {
            ...
            is Resource.Success -> {
                snackbarHostState.showSnackbar("PDF unlocked")
                password = ""
                viewModel.resetState()
            }
            else -> Unit
    ...
    Scaffold(
        ...
        Column(
            ...
            Button(
                onClick = { createDocumentLauncher.launch("unlocked_document.pdf") },
                enabled = selectedFile != null && password.isNotEmpty(),
                modifier = Modifier
                    .fillMaxWidth()
                    .padding(top = 16.dp)
            ) {
    ...
    (state as? Resource.Loading)?.let {
        ProgressDialog(title = "Unlocking...", progress = 0f, indeterminate = true)
    }
}
